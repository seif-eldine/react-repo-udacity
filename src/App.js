...
for(let i in movies) {
  	let currentMovie = movies[i]; // {id: 1,name: 'Planet Earth'}
  	let usersWhoFavoritedCurrentMovie = profiles.filter(p => p.favoriteMovieID == currentMovie.id);
	let finalObject = {
      name: currentMovie.name,
      likedBy: []
    };
    usersWhoFavoritedCurrentMovie.map(u => finalObject.likedBy.push(users[u.userID].name + ','));
...
<ul>
           {finalList.map(movie => <li>{movie.name} liked by : {movie.likedBy}</li>)}
        </ul>
      </div>
    );
  }
